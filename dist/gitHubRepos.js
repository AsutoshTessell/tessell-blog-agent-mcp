...
function githubBasicAuthHeader(token) {
...
export function publicHttpsCloneUrl(parsed) {
    return `https://github.com/${parsed.owner}/${parsed.repo}.git`;
}
export function displayRepoLabel(parsed) {
    return `${parsed.owner}/${parsed.repo}`;
}
export function resolveGitCacheRoot(mcpRoot) {
    const e = process.env.TESSELL_GIT_CACHE_DIR?.trim();
    if (e)
        return path.resolve(e);
    return path.join(mcpRoot, '.data', 'git-cache');
}
export function resolveMaxGithubRepos(explicit) {
...
export async function ensureGithubRepoCloned(options) {
    ...
    if (!parsed) {
        throw new Error(`Unsupported repo URL (use https://github.com/org/repo or git@github.com:org/repo.git): ${options.repoUrl}`);
    }
    ...
    try {
        await execFileAsync('git', [...gitGithubAuthConfigArgs(cwd, options.token), '-C', cwd, 'pull', '--ff-only'], { maxBuffer: 50 * 1024 * 1024, encoding: 'utf8', env });
    }
    catch {
        let branch = 'main';
        try {
            ...
            try {
                const { stdout } = await execFileAsync('git', ['-C', cwd, 'rev-parse', '--abbrev-ref', 'HEAD'], { encoding: 'utf8' });
                branch = stdout.trim() || 'main';
            }
            catch {
                branch = 'main';
        ...
        await execFileAsync('git', [...gitGithubAuthConfigArgs(cwd, options.token), '-C', cwd, 'reset', '--hard', `origin/${branch}`], { maxBuffer: 30 * 1024 * 1024, encoding: 'utf8', env });
    }
    return { cwd, label, publicUrl };
}
export function resolveMaxCommits(explicit) {
    if (typeof explicit === 'number' && !Number.isNaN(explicit) && explicit > 0) {
...
export async function readGitLogPretty(options) {
    ...
    if (options.onelineOnly) {
        const { stdout } = await execFileAsync('git', [...safeArgs, '-C', cwd, 'log', `--since=${options.daysBack} days ago`, '--no-merges', '--oneline', '-n', String(maxCommits)], { cwd, maxBuffer, encoding: 'utf8' });
        return stdout;
    }
...
export function repoDedupeKey(url) {
    const p = parseGithubRepoUrl(url);
    if (p)
        return `${p.owner}/${p.repo}`.toLowerCase();
