...
8. **Save, convert, publish (Sanity, then Hashnode per post)**  
   ...
   - **`publish_blog_to_hashnode`** (same absolute `markdownFilePath`): after each **successful** Sanity publish (`dryRun: false`), optional **`dryRun: true`** once to inspect `inputSummary`, then **`dryRun: false`**. **Default** is Hashnode **draft** (`createDraft`); use **`mode: publish`** for an immediate live post, or set **`HASHNODE_PUBLISH_MODE=publish`** in `.env`. **Skip** if Hashnode is not configured.
