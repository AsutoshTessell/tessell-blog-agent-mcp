...
import {
  ...
  PUBLICATION_BY_HOST_QUERY,
  hashnodeGraphqlRequest,
} from './hashnodeGraphql.js';
...
function normalizeTags(raw: unknown): Array<{ slug: string; name: string }> {
  if (!raw) return [];
...
export type PublishBlogToHashnodeMode = 'publish' | 'draft';

export type PublishBlogToHashnodeResult = {
  ok: true;
  dryRun: boolean;
  mode: PublishBlogToHashnodeMode;
  markdownFilePath: string;
  /** Input we would send (or sent) to Hashnode */
  inputSummary: {
    title: string;
    subtitle?: string;
    slug?: string;
...
type PublicationByHostData = {
  publication: { id: string; title?: string; url?: string } | null;
};

/** Public query — no PAT required. */
export async function fetchHashnodePublicationByHost(host: string): Promise<PublicationByHostData> {
  return hashnodeGraphqlRequest<PublicationByHostData>(
    PUBLICATION_BY_HOST_QUERY,
    { host: host.trim() },
    undefined
  );
 ...
 */
export async function resolveHashnodePublicationId(publicationHost?: string): Promise<string | undefined> {
 ...
 */
export function resolveHashnodeMode(explicit?: string): PublishBlogToHashnodeMode {
  const m = explicit?.trim().toLowerCase();
  if (m === 'publish') return 'publish';
...
export async function publishMarkdownToHashnode(options: {
  ...
  mode: PublishBlogToHashnodeMode;
  dryRun: boolean;
  /** When env has no HASHNODE_PUBLICATION_ID, pass host (e.g. tessell.hashnode.dev) to resolve id */
  publicationHost?: string;
}): Promise<PublishBlogToHashnodeResult> {
  ...
  if (options.mode === 'draft') {
    const result = await hashnodeGraphqlRequest<{
      createDraft: { draft: { id: string; title: string; slug?: string } };
    }>(CREATE_DRAFT_MUTATION, { input: baseInput }, token);
  ...
  }>(PUBLISH_POST_MUTATION, { input: baseInput }, token);
  ...
  return {
    ...
    reminder:
      'Post published on Hashnode. If this is syndication, ensure `canonicalUrl` / originalArticleURL points at your Tessell blog.',
  };
}
