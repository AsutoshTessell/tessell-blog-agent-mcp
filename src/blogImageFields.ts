...
/** Sanity `image` field value (asset lives in Content Lake). */
...
function imageFromAssetRef(ref: string): SanityImageField {
  return {
    _type: 'image',
 ...
 */
export function mergeBlogImages(
  ...
  let thumb =
    pickString(
      data,
      'thumbnailImageAssetRef',
      'thumbnail_image_asset_ref',
      'thumbnailAssetRef'
    ) || process.env.TESSELL_DEFAULT_THUMBNAIL_IMAGE_ASSET_REF?.trim();

  let main =
    pickString(data, 'mainImageAssetRef', 'main_image_asset_ref', 'mainAssetRef') ||
    process.env.TESSELL_DEFAULT_MAIN_IMAGE_ASSET_REF?.trim();

  if (main && !thumb) thumb = main;
  if (thumb && !main) main = thumb;
