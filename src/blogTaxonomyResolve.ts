...
export type TaxonomyRow = { _id: string; name: string; slug: string | null };

const UUID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

/** Sanity blogCategory / blogTag ids are typically 24-char hex or UUID-shaped strings. */
export function looksLikeSanityDocumentId(s: string): boolean {
  const t = s.trim();
...
function matchTaxonomyRow(rows: TaxonomyRow[], label: string): TaxonomyRow | undefined {
  const n = normLabel(label);
  const slugFromLabel = slugifyLabel(label);
  const trimmed = label.trim();
  return rows.find((r) => {
    if (normLabel(r.name) === n) return true;
    if (r.slug && (r.slug === trimmed || r.slug === slugFromLabel)) return true;
    return false;
  });
}

async function fetchTaxonomyRows(): Promise<{ categories: TaxonomyRow[]; tags: TaxonomyRow[] }> {
  const { client } = createSanityReadClientWithMeta({ useCdn: false });
  const [categories, tags] = await Promise.all([
    client.fetch(BLOG_CATEGORIES_QUERY) as Promise<TaxonomyRow[]>,
    client.fetch(BLOG_TAGS_QUERY) as Promise<TaxonomyRow[]>,
  ]);
  return {
 ...
 */
export async function resolveTaxonomyFromSanity(
  ...
  if (catRefRaw) {
    if (looksLikeSanityDocumentId(catRefRaw)) {
      ...
      if (m) catId = m._id;
        ...
        warnings.push(
          `blogCategoryRef "${catRefRaw}" did not match any category _id or name in Sanity (dataset refreshed on each conversion).`
        );
      }
    }
  }
  if (!catId && catLabel) {
    ...
    if (m) catId = m._id;
      ...
      warnings.push(
        `category "${catLabel}" did not match any blogCategory in Sanity. Call get_blog_categories_and_tags and use an exact name or slug from the response.`
      );
    }
  }
  ...
  const tagIdsOrdered: string[] = [];
  const seen = new Set<string>();

  const pushTagId = (id: string) => {
    if (!seen.has(id)) {
      seen.add(id);
  ...
  const blogTagsRefsList = parseRefList(
    frontmatter.blogTagsRefs ?? frontmatter.blog_tag_refs ?? frontmatter.blogTagIds ?? frontmatter.blog_tag_ids
  );
  ...
  for (const raw of blogTagsRefsList) {
    if (looksLikeSanityDocumentId(raw)) {
      ...
      if (m) pushTagId(m._id);
      else {
        warnings.push(
          `blogTagsRefs entry "${raw}" did not match any blogTag _id or name in Sanity.`
        );
      }
    }
  }
