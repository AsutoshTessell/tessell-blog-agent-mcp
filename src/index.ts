  ...
  return {
    tools: [
        ...
        inputSchema: {
          ...
          properties: {
            repoUrls: {
              type: 'string',
              description:
                'Optional comma-separated GitHub repo URLs. If omitted, uses `TESSELL_GITHUB_REPOS` from the MCP `.env`.',
            },
            daysBack: {
              type: 'number',
              description: 'Git log window in days (default: TESSELL_BLOG_DAYS_BACK env or 15)',
            ...
            maxRepos: {
              type: 'number',
              description:
                'Max number of repos to process after dedupe (default: TESSELL_GITHUB_MAX_REPOS env or 20, cap 100).',
            },
          },
        },
      },
      ...
      },
      {
        ...
        description:
          'Looks up a Hashnode publication by hostname (e.g. `tessell.hashnode.dev`) via public GraphQL — no token required. Returns `id` for HASHNODE_PUBLICATION_ID. See https://apidocs.hashnode.com/',
        inputSchema: {
          type: 'object',
          properties: {
            host: {
              type: 'string',
              description:
                'Publication host, e.g. `your-team.hashnode.dev`. Optional if HASHNODE_PUBLICATION_HOST is set in tessell-blog-agent-mcp/.env.',
            },
          },
        },
      },
        ...
        inputSchema: {
          ...
          properties: {
            ...
            publicationHost: {
              type: 'string',
              description:
                'Optional publication hostname when HASHNODE_PUBLICATION_ID is unset (used to query publication id).',
            },
          },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (urls.length === 0) {
        throw new McpError(
          ...
          'No GitHub repos configured: set TESSELL_GITHUB_REPOS in tessell-blog-agent-mcp/.env (comma-separated https://github.com/org/repo URLs) or pass repoUrls.'
        );
      }
      ...
      for (const u of urls) {
        ...
        seen.add(key);
        uniqueUrls.push(u);
        if (uniqueUrls.length >= maxRepos) break;
      }
  ...
  if (request.params.name === 'resolve_hashnode_publication') {
    const args = (request.params.arguments || {}) as { host?: string };
    const host = args.host?.trim() || process.env.HASHNODE_PUBLICATION_HOST?.trim();
    if (!host) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Provide `host` (e.g. tessell.hashnode.dev) or set HASHNODE_PUBLICATION_HOST in tessell-blog-agent-mcp/.env'
      );
    }
    try {
