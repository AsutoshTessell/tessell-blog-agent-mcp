  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description:
          'Lists published blog posts from Sanity using the same project/dataset as tessell-web (env: SANITY_PROJECT_ID, SANITY_DATASET default staging) and the same filters as the website blog listing (non-draft, non-archived, perspective published).',
        inputSchema: {
          type: 'object',
          properties: {},
        },
            ...
            if (res.statusCode && (res.statusCode < 200 || res.statusCode >= 300)) {
              reject(
                new McpError(
                  ...
                  `Sanity HTTP ${res.statusCode}: ${data.slice(0, 500)}`
                )
              );
              return;
            }
